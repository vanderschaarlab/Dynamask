...
from dynamask.utils.metrics import get_entropy_array, get_information_array
...
def process_results(CV, explainer_list):
    ...
    for cv in range(CV):
        with open(
            f"experiments/results/rare_feature/true_saliency_{cv}.pkl", "rb"
        ) as f:
            true_saliency = pkl.load(f).cpu().numpy()
        for e, explainer in enumerate(explainer_list):
            with open(
                f"experiments/results/rare_feature/{explainer}_saliency_{cv}.pkl", "rb"
            ) as f:
                pred_saliency = pkl.load(f).cpu().numpy()
            prec, rec, thres = precision_recall_curve(
                true_saliency.flatten(), pred_saliency.flatten()
            )
            ...
            sub_saliency = pred_saliency[
                true_saliency != 0
            ]  # This is the saliency scores for each truly salient input
